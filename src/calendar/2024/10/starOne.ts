...
export async function exec(fileName: string): Promise<number> {
  ...
  for (let trailHeadIndex = 0; trailHeadIndex < possibleTrailheads.length; trailHeadIndex++) {
    ...
    result += pathLookup(currentPosition, matrix, new Set<string>());
  }

  return result;
};

const pathLookup = (currentPosition: Coordinate, map: string[][], visitedSet: Set<string>): number => {
  const key = `${currentPosition[0]},${currentPosition[1]}`;

  if (visitedSet.has(key)) {
    //Already checked - no more score
    return 0;
  }
  ...
  if(nextPossibleSteps.length > 0) {
    return R.sum(R.map((nextPossibleStep) => pathLookup(nextPossibleStep, map, visitedSet), nextPossibleSteps));
  }
